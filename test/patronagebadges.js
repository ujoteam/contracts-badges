...
contract('Patronage Badges', (accounts) => {
  beforeEach(async () => {
    ...
    // eslint-disable-next-line max-len
    badges = await ujoBadges.new(accounts[0], oracle.address, '0x0', { gas: parseInt((gasEstimate * 110) / 100, 0), from: accounts[0] });
  });

  it('minting: mint and test events', async () => {
    const result = await badges.mint(accounts[0], 'cidcidcidcidcidcidcidcid1', 'qmxnftqmxnftqmxnftqmxnft1', accounts[1], 5, { from: accounts[0], value: web3.utils.toWei('6', 'ether') });
    const jscomputedid1 = web3.utils.soliditySha3('cidcidcidcidcidcidcidcid1', 'qmxnftqmxnftqmxnftqmxnft1', accounts[1], 5, 1);
  ...
  it('minting: should send the right amount of money to the benficiary and to the payer', async () => {
    ...
    const result = await badges.mint(accounts[0], 'cidcidcidcidcidcidcidcid', 'qmxnftqmxnftqmxnftqmxnft', accounts[1], 5, { from: accounts[0], value: web3.utils.toWei('6', 'ether') });
  ...
  it('minting: should fail if amount is less than specified USD (5 usd, paying 4 usd).', async () => {
    await assertRevert(badges.mint(accounts[0], 'cidcidcidcidcidcidcidcid', 'qmxnftqmxnftqmxnftqmxnft', accounts[1], 5, { from: accounts[0], value: web3.utils.toWei('4', 'ether') }));
  });
  ...
  it('minting: should fail if exchange rate is 0', async () => {
    ...
    await assertRevert(badges.mint(accounts[0], 'cidcidcidcidcidcidcidcid', 'qmxnftqmxnftqmxnftqmxnft', accounts[1], 5, { from: accounts[0], value: web3.utils.toWei('5', 'ether') }));
  });

  it('minting/burning: create 5 tokens, burn 3, create 3 more.', async () => {
    await badges.mint(accounts[0], 'cidcidcidcidcidcidcidcid', 'qmxnftqmxnftqmxnftqmxnft', accounts[1], 5, { from: accounts[0], value: web3.utils.toWei('5', 'ether') });
    await badges.mint(accounts[0], 'cidcidcidcidcidcidcidcid', 'qmxnftqmxnftqmxnftqmxnft', accounts[1], 5, { from: accounts[0], value: web3.utils.toWei('5', 'ether') });
    await badges.mint(accounts[0], 'cidcidcidcidcidcidcidcid2', 'qmxnftqmxnftqmxnftqmxnft2', accounts[2], 5, { from: accounts[0], value: web3.utils.toWei('5', 'ether') });
    await badges.mint(accounts[0], 'cidcidcidcidcidcidcidcid2', 'qmxnftqmxnftqmxnftqmxnft2', accounts[2], 5, { from: accounts[0], value: web3.utils.toWei('5', 'ether') });
    await badges.mint(accounts[0], 'cidcidcidcidcidcidcidcid3', 'qmxnftqmxnftqmxnftqmxnft3', accounts[3], 5, { from: accounts[0], value: web3.utils.toWei('5', 'ether') });
    assert.equal(await badges.totalSupply(), 5);
    const jscomputedid1 = web3.utils.soliditySha3('cidcidcidcidcidcidcidcid', 'qmxnftqmxnftqmxnftqmxnft', accounts[1], 5, 1);
    const jscomputedid2 = web3.utils.soliditySha3('cidcidcidcidcidcidcidcid', 'qmxnftqmxnftqmxnftqmxnft', accounts[1], 5, 2);
    const jscomputedid3 = web3.utils.soliditySha3('cidcidcidcidcidcidcidcid2', 'qmxnftqmxnftqmxnftqmxnft2', accounts[2], 5, 1);
  ...
  it('minting: mint 1 token for another buyer', async () => {
    await badges.mint(accounts[2], 'cidcidcidcidcidcidcidcid', 'qmxnftqmxnftqmxnftqmxnft', accounts[1], 5, { from: accounts[0], value: web3.utils.toWei('5', 'ether') });
    const jscomputedid1 = web3.utils.soliditySha3('cidcidcidcidcidcidcidcid', 'qmxnftqmxnftqmxnftqmxnft', accounts[1], 5, 1);
  ...
  it('minting: mint should fail due to overflow in multiplication', async () => {
    await assertJump(badges.mint(accounts[2], 'cidcidcidcidcidcidcidcid', 'qmxnftqmxnftqmxnftqmxnft', accounts[1], '115792089237316195423570985008687907853269984665640564039457584007913129639935', { from: accounts[0], value: web3.utils.toWei('5', 'ether') }));
  });

  it('minting: should store & get all tokens by an owner', async () => {
    await badges.mint(accounts[0], 'cid', 'qmxnftqmxnftqmxnftqmxnft', accounts[1], 5, { from: accounts[0], value: web3.utils.toWei('5', 'ether') });
    await badges.mint(accounts[0], 'cid2', 'qmxnftqmxnftqmxnftqmxnft2', accounts[2], 5, { from: accounts[0], value: web3.utils.toWei('5', 'ether') });
    ...
    const id1 = web3.utils.soliditySha3('cid', 'qmxnftqmxnftqmxnftqmxnft', accounts[1], 5, 1);
    const id2 = web3.utils.soliditySha3('cid2', 'qmxnftqmxnftqmxnftqmxnft2', accounts[2], 5, 1);
    const array = [id1, id2];
  ...
  it('URI: should allow setting URI ID & fail when set by non-admin', async () => {
    await badges.mint(accounts[0], 'cidcidcidcidcidcidcidcid', 'qmxnftqmxnftqmxnftqmxnft', accounts[1], 5, { from: accounts[0], value: web3.utils.toWei('5', 'ether') });
    const id = web3.utils.soliditySha3('cidcidcidcidcidcidcidcid', 'qmxnftqmxnftqmxnftqmxnft', accounts[1], 5, 1);
    await badges.setTokenURIID(id, 'qmxnftqmxnftqmxnftqmxnft2', { from: accounts[0] });
  ...
  it('URI: should allow setting URI Base & fail when set by non-admin', async () => {
    await badges.mint(accounts[0], 'cidcidcidcidcidcidcidcid', 'qmxnftqmxnftqmxnftqmxnft', accounts[1], 5, { from: accounts[0], value: web3.utils.toWei('5', 'ether') });
    const id = web3.utils.soliditySha3('cidcidcidcidcidcidcidcid', 'qmxnftqmxnftqmxnftqmxnft', accounts[1], 5, 1);
    await badges.setTokenURIBase('new_base?=', { from: accounts[0] });
    const newURI = await badges.tokenURI.call(id);
  ...
  it('URI: should allow setting URI Suffix & fail when set by non-admin', async () => {
    await badges.mint(accounts[0], 'cidcidcidcidcidcidcidcid', 'qmxnftqmxnftqmxnftqmxnft', accounts[1], 5, { from: accounts[0], value: web3.utils.toWei('5', 'ether') });
    const id = web3.utils.soliditySha3('cidcidcidcidcidcidcidcid', 'qmxnftqmxnftqmxnftqmxnft', accounts[1], 5, 1);
    await badges.setTokenURISuffix('.json', { from: accounts[0] });
    const newURI = await badges.tokenURI.call(id);
  ...
  it('URI: should revert if URI ID set for a token that doesnt exist', async () => {
    await badges.mint(accounts[0], 'cidcidcidcidcidcidcidcid', 'qmxnftqmxnftqmxnftqmxnft', accounts[1], 5, { from: accounts[0], value: web3.utils.toWei('5', 'ether') });
    await assertRevert(badges.setTokenURIID(32, 'newcid', { from: accounts[0] }));
  });
  ...
  it('URI: test locks on ID, Base & Suffix setting', async () => {
    ...
    await badges.lockAdmin({ from: accounts[0] });
    await assertRevert(badges.setTokenURIID(id, 'qmxnftqmxnftqmxnftqmxnft2', { from: accounts[0] }));
    await assertRevert(badges.setTokenURIBase('new_base?=', { from: accounts[0] }));
    await assertRevert(badges.setTokenURISuffix('.json', { from: accounts[0] }));
  });
