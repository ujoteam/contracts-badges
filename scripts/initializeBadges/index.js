...
const getDataFromApi = async formattedData =>
  Promise.all(formattedData.map(async (singleBadgeData) => {
    ...
    const recipientsHash = web3.utils.soliditySha3({ type: 'address', value: [beneficiaryOfBadge] }, { type: 'uint256', value: [100] });
    const objectForIPFS = {
...
const writeData = async (dataWithCids) => {
  let functionsToWrite = '// function createBadge(address _buyer, string _mgCid, string _nftCid, bytes32 _recipientsHash, uint256 _usdCost)\n';
  dataWithCids.forEach(({
    buyer, recipientsHash, MusicGroup, nftCid,
  }) => {
    functionsToWrite += `address(this).delegatecall(abi.encodeWithSignature("adminCreateBadge(address,string,string,bytes32,uint256)", ${buyer}, "${MusicGroup}", "${nftCid}", ${recipientsHash}, 5));\n`;
  });
